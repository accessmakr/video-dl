/**
 * App.jsx
 * Root component — social media video downloader + audio converter.
 */
...
export default function App() {
  ...
  // Track the current download result URL + name for passing to AudioConverter
  ...
  // Fetch preview thumbnail when platform URL is detected
  useEffect(() => {
  ...
  // When a stream result arrives, expose the URL to the AudioConverter
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white flex flex-col items-center px-4 py-12 gap-8">

      {/* ── Header ── */}
      <div className="text-center max-w-xl">
        <h1 className="text-3xl font-bold mb-2">Video Downloader</h1>
        <p className="text-zinc-400 text-sm mb-4">
          Download videos and extract audio from Facebook, X (Twitter) and Instagram
        </p>
        ...
        {platform && (previewing || preview !== null) && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden flex gap-3 p-3 items-center">
            {previewing && !preview?.thumbnail ? (
              <div className="w-28 h-16 bg-zinc-800 rounded-lg flex-shrink-0 animate-pulse" aria-hidden="true" />
            ) : preview?.thumbnail ? (
            ...
            <div className="flex flex-col gap-1 overflow-hidden flex-1">
              {previewing && !preview?.title ? (
                <div className="h-4 bg-zinc-800 rounded animate-pulse w-3/4" aria-hidden="true" />
              ) : (
                <p className="text-white text-sm font-medium line-clamp-2">
                  {preview?.title || platform.label + ' Video'}
                </p>
              )}
              <span className={`text-xs ${platform.color}`} aria-hidden="true">
      ...
      {/* Error */}
      {error && (
        <div className="w-full max-w-xl bg-red-950 border border-red-800 rounded-xl px-4 py-3 text-red-400 text-sm text-center" role="alert">
          {error}
        </div>
      )}
      ...
      {/* ── Divider ── */}
      <div className="w-full max-w-xl border-t border-zinc-800" aria-hidden="true" />
      ...
      {/* SEO footer */}
      <footer className="mt-auto pt-8 text-center text-zinc-700 text-xs max-w-xl space-y-1" aria-label="Site tags">
