...
// ── ffmpeg loaded lazily from CDN the first time the user taps Extract MP3 ──
const ffmpegCache = { instance: null, fetchFile: null };

async function loadFFmpeg(onStatus) {
  if (ffmpegCache.instance) return ffmpegCache;
  ...
  ffmpegCache.instance  = ff;
  ffmpegCache.fetchFile = fetchFile;
  return ffmpegCache;
}
...
function Mp3Converter() {
  ...
  const convert = async () => {
    ...
    setProgress(0);
    try {
      const { instance: ff, fetchFile } = await loadFFmpeg(setStatusMsg);
  ...
  return (
    <div className="w-full max-w-xl border border-zinc-800 rounded-xl p-4 flex flex-col gap-3">
      ...
      <label className="border-2 border-dashed border-zinc-700 hover:border-zinc-500 rounded-xl p-5 text-center cursor-pointer transition-colors">
        <input
          type="file"
          accept="video/*,audio/*,.mp4,.mov,.avi,.webm,.mkv"
          onChange={handleFile}
          className="hidden"
        />
        {file ? (
      ...
      {statusMsg && (
        <p className="text-zinc-400 text-xs text-center">{statusMsg}</p>
      )}
